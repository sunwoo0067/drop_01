...
class AIService:
    ...
    async def plan_seasonal_strategy(self, context_products: Optional[List[Dict[str, Any]]] = None, provider: ProviderType = "auto") -> Dict[str, Any]:
        ...
        prompt = f"""
        당신은 대한민국 이커머스 시장에 정통한 전문 MD 겸 전략가입니다.
        오늘 날짜({current_date})를 기준으로 현재 시즌 및 다가오는 이벤트를 정밀 분석하여 소싱 및 운영 전략을 수립해주세요.

        [중요 지침: 날짜 엄수]
        - 오늘({current_date})은 12월 말입니다. 크리스마스({current_date.split('-')[0]}-12-25)는 이미 종료되었습니다.
        - 소싱 리스트와 다가오는 이벤트에서 이미 종료된 이벤트(Christmas, 성탄절 등)를 절대 포함하지 마세요.
        - 지금부터 준비해야 할 것은 '신년(새해) 맞이', '겨울 방학', '설날(구정)', '졸업 및 입학 시즌'입니다.

        [분석 및 소싱 요청 사항]
        1. 현재 시즌 이름 및 다가오는 대한민국 주요 이벤트를 식별하세요 (예: 설날, 신학기, 한파 대비).
        2. '집중 소싱 키워드' 리스트를 생성하세요. 
           - 비율: 다가오는 이벤트 관련 50% + 카테고리별 스테디셀러(생활가전, 주방, 생활잡화, 반려동물 등) 50%.
           - 총 30개 이상의 구체적이고 구매 전환율이 높을만한 키워드를 도출하세요.
        3. 수명이 다했거나 시즌이 지나 즉시 삭제/정리해야 할 '아웃데이트 키워드' 리스트를 생성하세요. 
           - (중요: 크리스마스 장식, 트리, 성탄절 선물 등은 이제 아웃데이트에 포함되어야 합니다.)
        4. 상품 가공 및 마케팅 시 강조해야 할 '핵심 테마'를 선정하세요.

        결과는 반드시 다음과 같은 JSON 구조로만 답변하세요:
        {{
            "season_name": "...",
            "upcoming_events": ["설날 준비", "신학기 준비", ...],
            "target_keywords": ["방한 용품", "설날 선물세트", ...],
            "out_dated_keywords": ["크리스마스 트리", "성탄절 장식", ...],
            "strategy_theme": "...",
