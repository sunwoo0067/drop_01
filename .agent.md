...
프로젝트의 모든 작업은 Jira 이슈를 기반으로 관리됩니다.

### Jira 활용 수칙
- **이슈 기반 작업**: 모든 신규 기능 개발(`feat`) 및 버그 수정(`fix`)은 Jira 이슈가 선행되어야 합니다.
- **Jira MCP 활용**: 작업 전 `list_issues`나 `search_issues`를 통해 현재 할당된 작업을 확인하고, 작업 진행 상태를 업데이트합니다.
- **상태 업데이트**: 작업 시작 시 이슈 상태를 `In Progress`로, 완료 시 `Done`으로 변경하며 필요한 경우 코멘트를 남깁니다.
