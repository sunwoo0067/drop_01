...
async def test_ai():
    ...
    specs = await service.extract_specs(text, provider="auto")
    logger.info(f"Result: {specs}")

    # 2. Test Gemini Explicitly
    logger.info("Testing 'analyze_pain_points' with provider='gemini'")
    points = await service.analyze_pain_points(text, provider="gemini")
    ...
    specs_ollama = await service.extract_specs(text, provider="ollama")
    logger.info(f"Result: {specs_ollama}")

    # 4. Test OpenAI Explicitly
    logger.info("Testing 'extract_specs' with provider='openai'")
    specs_openai = await service.extract_specs(text, provider="openai")
